...
// generic builds (distribution.ini holds ENTERPRISE_CONSOLE_PLACEHOLDER and
// nothing is persisted yet). The dialog stores the address in felt.json, then
...
#if defined(MOZ_ENTERPRISE)
// Set when distribution.ini holds ENTERPRISE_CONSOLE_PLACEHOLDER and no
...
#  endif
  ...
  // Otherwise ignoring nsresult. If console url is not found in a release
  // build, the default server url is empty and crash reports will fail to
  // submit.
#endif
...
#if defined(MOZ_ENTERPRISE)
  if (gEnterpriseConsoleSetupNeeded && is_felt_ui()
#  ifdef MOZ_BACKGROUNDTASKS
      && !BackgroundTasks::IsBackgroundTaskMode()
#  endif
  ) {
